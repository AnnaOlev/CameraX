...
class PopUpWindow : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        popupText = bundle?.getString("result", "Text") ?: ""
        ...
        popup_window_button_repeat.setOnClickListener {
            makeSound()
        }
    ...
    private fun makeSound() {
        textToSpeechObject?.speak(popupText, TextToSpeech.QUEUE_FLUSH, null)
    }
    ...
    override fun onInit(status: Int) {
            if (status == TextToSpeech.SUCCESS) {
                if (textToSpeechObject?.isLanguageAvailable(Locale(Locale.getDefault().language))
                    == TextToSpeech.LANG_AVAILABLE)
                {
                    textToSpeechObject?.language = Locale(Locale.getDefault().language)
                } else {
                    textToSpeechObject?.language = Locale.US
                }
                textToSpeechObject?.setPitch(1.3f)
